...
// When DATABASE_URL / Supabase env is missing or still placeholder text, skip
// Neon and let better-auth use its in-memory adapter so local signup works.
const demoAuth = shouldUseDemoAuth();

const pool =
  !demoAuth && process.env.DATABASE_URL
    ? new Pool({
...
if (demoAuth) {
  console.warn(
    '[auth] Demo/memory auth enabled — DATABASE_URL or Supabase env is missing/dummy. Accounts are in-memory only.'
  );
}
  ...
  new Set(
    [
      ...
      process.env.NEXT_PUBLIC_CREATE_HOST
        ...
        : null,
      // Local Next.js dev (http) — required for CSRF when testing signup locally.
      'http://localhost:4000',
      'http://127.0.0.1:4000',
      'http://localhost:3000',
      'http://127.0.0.1:3000',
...
// Demo/local http://localhost cannot store Secure + SameSite=None cookies.
// Keep iframe-safe cookies for real hosted environments.
